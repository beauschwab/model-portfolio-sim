...
def run_balance_sheet_nii(bs: dict, swap_rates, vol_pts, dep_hist,
    ...
    from .corp import CorpDeck, _corp_full
    ...
    from .config import SWAP_TENORS, N_PATHS_SENS
    ...
    if "mbs" in bs:
        from .scenarios import (build_paths, run_engine, setup,
    ...
    if bs.get("mm") is not None:
        from .mm import MMDeck, mm_earning_assets, mm_income
        deck = MMDeck(bs["mm"])
    ...
    if bs.get("hedges") is not None:
        from .hedges import HedgeDeck, swap_mtm_and_carry
        swaps, _ = bs["hedges"]
